...
#include <pybind11/stl.h> // Crucial: Converts std::vector and std::variant automatically

namespace py = pybind11;

// "uwudb" will be the name of the Python module you import
PYBIND11_MODULE(uwudb, m) {
    m.doc() = "Custom C++ B+ Tree Database Engine";

    py::class_<Row>(m, "Row")
        .def(py::init<>())
        .def_readwrite("columns", &Row::columns)
        .def_readwrite("isActive", &Row::isActive);

    // Bind the Tree class
    py::class_<Tree>(m, "Tree")
        .def(py::init<int>()) // Allow Python to do: db = Tree(5)
        .def("insert", &Tree::insert)
        .def("dataSearch", &Tree::dataSearch)
        .def("updateRow", &Tree::updateRow)
        .def("remove", &Tree::remove)
        .def("display", &Tree::display); // Helpful for backend terminal debugging
}